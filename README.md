...
1. [ ] **Export All should offer video as `.mp4`** — Export All pages over raw database rows rather than the grouped slots the grid shows, so a project with five clips writes ~1,285 loose frames named by rowid with no frame numbers. The pieces to fix it exist (`collapseIntoSlots` for the grouping, the `.movie` path from Export Series for the assembly); the work is walking slots instead of rows and adding a chooser. **The audio question is now settled** — `VideoAssembler` muxes the clip's Draw Things soundtrack as AAC, so a single-clip `.mp4` export already carries sound and a bulk one will too; nothing left to decide before making it a bulk operation. Keep the byte-exact frame export alongside it, since an `.mp4` is a lossy re-encode and can't give an exact frame back
...
3. [ ] **Story Studio configs — make the base config easy to change** — a Story Studio project's Base Config is free-text JSON, and the default new projects start from is a Swift string literal in `StoryStudioModels.swift`, so tuning it needs a rebuild. Meanwhile the app already holds **16 saved configs** (`ZIT1.0`, `flux-default`, `cyberrealpony`, `qwen-image`, `turbo-fast`, …) as StoryFlow config variables, with a storage API (`StoryFlowStorage.loadVariables` / `saveVariable`) that Story Studio does not use. `StoryFlowCompiler` already wraps the base config into a `WorkflowVariable(type: .config)` named `storyBase` before handing it to the engine, so Story Studio speaks the same currency — it synthesises a private variable where it could reference a shared one, and `applyConfigVar` already resolves those. Four steps, in dependency order:
     1. **"Use a saved config…" in Project Info** — a menu of the saved configs that fills Base Config in. Small, uses storage that already exists, and on its own makes the field easy to change
     2. **Make the new-project default a setting** — a `tanqueStudio.*` key naming which saved config new projects start from, replacing the hard-coded literal. Retires the rebuild-to-tune problem, which is the actual complaint
     3. **"Use as Story Studio base" wherever a config already exists** — the DT Project Browser's Copy Config, Generate, a StoryFlow config variable. Once step 1 exists these are all the same mechanism, so they are cheap. Note the browser's `configJSON(for:)` already emits camelCase JSON that `mergeDict` reads, so copy-and-paste into Base Config works *today*, undocumented
     4. **"Send to new Story Studio project" from a render** — start a story from an image you liked. The only genuinely new UI here, and the only optional one

     **⚠️ Decide snapshot vs live reference before building step 1, because it is hard to unwind.** If a project *references* a saved config, editing that config later silently changes what every existing project renders — which collides with the reproducibility work (the /64 floor, metadata that describes its own PNG) that is premised on a stored config describing what actually happened. **Recommendation: snapshot** — picking a saved config copies its JSON in, so Project Info keeps showing what will actually render, old projects never change underneath you, and it is additive to the existing field rather than replacing it. A live-link mode can be added later; removing one cannot.
4. [ ] **Generate drops most of an imported image's settings** — dragging a rendered image into Generate restores only **10 of the config's 58 fields**: model, sampler, steps, guidanceScale, seed, seedMode, width, height, shift, strength (`GenerateViewModel.applyMetadataToConfig`). `PNGMetadata` parses roughly 44, so LoRAs, hires fix, tiling, refiner, negative prompt, CFG-Zero, resolution-dependent shift and the SDXL conditioning fields are all read and then discarded. Reported against 0.9.31; dtm restores everything. **The model list is not the cause** — the two model gates warn and apply anyway ("isn't in the local model list — generating anyway (may be a DT+ cloud model)"), deliberately, so DT+ cloud models still work. Two parts: widen the applier to cover every field both sides model, and **add a raw-JSON metadata view** — an expandable panel showing exactly what was parsed out of the file, independent of what Generate chose to apply. The viewer is worth building first: it is the diagnostic that would have identified this in a minute rather than by reading the applier
5. [ ] **Render queue — matrix in, job list out** (replaces the Labs "Workflow Builder / Coming soon" stub, which currently promises a tool that was deleted in the v2 scaffold, `09ea9cd`, and survives only on `origin/archive/v0.9.x`). The goal is the thing Draw Things can only do by scripting: a series that tries **various prompts across different models, LoRAs and settings**, set up once and left to run. Design settled as: **define a matrix of axes → Expand → a flat queue of concrete jobs → prune, reorder, pause, run.**
     - **Each queued job carries a FULL standalone config and prompt**, not a base plus overrides. Overrides may exist during construction, but what lands in the queue is self-contained — so a row always describes exactly what it will render, and stays reproducible after the matrix that generated it changes
     - **Expanding to concrete jobs is what makes LoRAs a usable axis.** `sweep` cannot vary them (`loras` is an array of objects; sweep cards are scalars, and it is absent from `sweepableParameters`), but an expander writes the `loras` array straight into each job's config. `model` *is* already sweepable, so models alone could be done with nested loops today — the gap is authoring, not capability
