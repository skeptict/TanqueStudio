...
- Live per-character readout of spoken word count, frame count and duration, shown before anything renders. There is no frame cap in either engine — Tanque Studio used to clamp at 257 while Draw Things' pipeline did not, which made one project render two different lengths; the real limit is what a given model at a given canvas size will render, so the number is surfaced rather than guessed at
