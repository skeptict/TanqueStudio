...
Authoring for a two-phase StoryFlow project — one that renders a cast of character stills, then uses each still as the first frame of that character's spoken clip.

- **New Project…** creates the folder and seeds both files, so a project starts in the app rather than by copying someone else's folder and editing it. The prose arrives with the spaces the format requires already correct — the one thing nobody should have to get right by typing — and the two Draw Things configs are assigned from your own saved `#config` variables, matched by name where the intent is unambiguous and left blank where it isn't
- Edits a project folder's `bible.json` and `configs.json`: a **cast table** and a **staging** panel holding each phase's prompt, the negative prompt, the two canvas sizes and the dialogue pacing
- **The fields are yours.** Each phase's prompt is an ordered list of *prose* and *columns* — prose is shared by every character, a column is one per-character field. Adding a column to a phase puts a field on every cast card; renaming one relabels every row without moving a character of text; reordering reorders the prompt and the table together. There is no separate palette of field names to keep in step with the prompt, because the prompt **is** the declaration. A column used by both phases is one column, which is what makes the two phases stay in lockstep
- Mark a column **spoken** and the emitter wraps it in quotes — spoken words are the only ones `framesDialog` counts, so stage direction can be as long as it needs to be
- Emits the StoryFlow project and its Draw Things instruction array from those two files. There is deliberately **no editor for the emitted project** — hand-editing it is how this format's two silent failures get in, and neither errors: a wildcard value that loses its innermost quotes makes every clip render at exactly the padding length, and identity/wardrobe lists that drift apart between the two phases dress every character as somebody else
- Each cast row previews the **assembled prompt** it produces, per phase — the string the model actually reads, with the quoted spans visible. That is what the format's spacing rules are standing in for, so it replaced a set of per-fragment space markers that could only exist while the fragment names were fixed
- Live per-character readout of spoken word count, resulting frame count and duration, calling out the one case where the two engines would render *different* lengths. Tanque Studio caps the spoken frame count at 257 **before** padding is added and Draw Things' pipeline has no cap at all, so they agree until the pre-padding count passes 257 — 27 spoken words at the default pacing, and the row then shows both numbers rather than only its own
- Validation in the panel rather than in a terminal, covering every silent failure the format has: non-`loop` wildcards, unequal card counts, zero quoted spans, unparseable object values, instruction types outside the pipeline's 52-key table, padding that isn't a multiple of 8, canvases whose aspect ratios differ, a `loop` without `start: 0`, duplicate pinned seeds, and a stray `"` in any bible field or fragment. A failure blocks emission
- Fragment spacing is enforced in the form, not merely validated after it: `concat` appends with no separator, so each fragment shows a marker for the leading or trailing space it needs and the marker turns red on the keystroke that removes it
...
│   ├── StoryFlowLoopPaths.swift       # loopSave/loopLoad path + directory-index rules, ported exactly
│   └── StoryFlowView/ViewModel/*Panel.swift
│
├── Cast & Staging (Labs)
│   ├── StoryFlowCastProject.swift     # bible.json + configs.json as one document, unknown blocks preserved
│   ├── StoryFlowCastEmitter.swift     # cast + staging → StoryFlowProject, and the two emitted files
│   ├── StoryFlowCastValidator.swift   # Every silent failure the format has, surfaced in the panel
│   ├── StoryFlowOrderedJSON.swift     # Order-preserving JSON; key order is part of the artifact here
│   └── StoryFlowCastPane/ViewModel.swift
│
├── Story Studio (Labs)
...
- [x] **StoryFlow Phase 4/5/polish, all three** — `enhance` executes natively, reusing the same `LLMService` call Story Studio's scene fields already make, and drops out of preflight's skipped list; `removeBkgd` and `maskFG` execute via `VNGenerateForegroundInstanceMaskRequest` on the live canvas, following the engine's existing `.crop` pattern; and a `.image`-type StoryFlow variable can finally have an image attached to it at all — there had been no drag-drop *and* no file picker, so the field the engine reads was unreachable from the UI. `interrogate` and the remaining Vision ops stay deferred for the reasons in Backlog: they need a multimodal LLM call that doesn't exist yet, or an API that doesn't cover the operation
- [x] **Multi-loop projects run correctly** — two real divergences from Draw Things' own pipeline, both of which only bite on a project with more than one loop block, which is why nothing had caught them. `globalLoopCounter` was never reset when a loop depleted, so a second loop block started at the first one's final count and every pass drew the *previous* item's card — six plausible, finished, uniformly mismatched renders with no warning. And `loopSave`/`loopLoad` had no executor at all, so a project that saved stills in one loop and read them back in the next rendered the first half and silently saved none of it. Both fixed, with `getDirectoryByIndex`'s numeric-then-alphabetical sort ported exactly rather than approximated — a different tie-break silently changes which saved image pairs with which pass. One deliberate deviation: loop paths resolve against the run's own timestamped output folder rather than `~/Pictures`, following the existing canvas save/load precedent, so a two-phase project must be run in a single pass in Tanque Studio
- [x] **Cast & Staging (Labs) — authoring a two-phase project without hand-writing JSON** — a cast table and a staging panel over a project folder's `bible.json` + `configs.json`, emitting the StoryFlow project and its Draw Things instruction array from them. The design decision underneath it is that the bible stays canonical and the emitted `.json` stays a build artifact: there is no editor for the emitted project, because hand-editing it is the only way this format's two silent failures get in. Validation ports every check into the panel, and a failure blocks emission rather than warning, since each one produces a finished, plausible-looking, wrong set of renders. The Python generator stays as the headless path and the fixture writer, pinned to the Swift emitter by a test that compares the emitted files **byte for byte** — a structural comparison would pass while the two disagreed on key order inside a wildcard's value string, and that string is the artifact
...
- [ ] **Cast & Staging: preserve the source files' own formatting on save** — `bible.json` and `configs.json` are hand-authored documents carrying long `_schema` prose blocks, and writing them back through a structured serializer normalizes their whitespace: blank lines between blocks go, inline objects expand. Content, keys and key order are byte-identical either way (asserted by `testSavingAnUntouchedDocumentChangesNothing`) — only the hand-alignment changes, about 22 cosmetic lines on `configs.json`. Emitting a project doesn't touch the source files at all; this only bites on **Save Source**. Fixing it means editing the source text in place rather than re-emitting the document
