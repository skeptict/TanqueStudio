...
1. [ ] **Warn before a run that will skip steps** — a StoryFlow project can contain instructions Tanque Studio preserves but cannot yet execute. Today those are skipped with only a line in the run log, which is fine for a background-mask step and badly wrong for one that supplies the prompt: a real project whose subject comes entirely from `concat`/`wildcard` steps renders its camera directions alone. Count and name what will be skipped, before the run starts
2. [ ] **Config parity Batch D — tiling** (8 fields) — unblocked now the client bump has landed, and a real StoryFlow project already depends on it: a tiled-decoding character-sheet project carries all eight fields, which Tanque Studio currently drops on import
3. [ ] **StoryFlow test hardening** — check the character-sheet project in as a third round-trip fixture, and migrate the standalone round-trip harness (12 checks, including the diff against the format author's own export) into the `TanqueStudioTests` target so it runs with everything else
4. [ ] **StoryFlow design pass** — bring StoryFlow into the Dashboard's design language before Phase 2 doubles its step-type count, so the new step cards are authored once rather than built and then restyled
5. [ ] **StoryFlow 260723 Phase 2 — author the new instructions** — `concat`, `wildcard`, `sweep`, `enhance`, `approve`, `framesDialog`, `size`, plus the long-passthrough backlog (`negPrompt`, `frames`, `adaptSize`, moodboard weights/remove, `inpaintTools`, `xlMagic`, mask and loop file ops). Takes authorable coverage from 14 of 49 instructions to ~38. Export-only — these run in Draw Things. `interrogate` is deliberately excluded and stays passthrough-only
6. [ ] **Video handling** — make a frame series read as one thing rather than N loose images. In the DT Project Browser, collapse a series into a single frame-count-badged reference that expands in place to show its frames. Draw Things turns out to expose a real series key (`clip_id` + `index_in_a_clip`, plus a `Clip` table carrying count and fps), so this is exact grouping, not a heuristic — verified against real projects, one of which is 1285 video frames vs 151 stills. Plus a general pass over video handling
7. [ ] **StoryFlow 260723 Phase 3 — native execution** — run the DT-model-free subset in-app: run-time wildcards and parameter sweeps (stateful loop/once/shuffle/random trackers), `size`, `frames`, `framesDialog`, `negPrompt`, `adaptSize`, moodboard weights, `inpaintTools`, and a human-in-the-loop `approve` sheet. Adopts the new `concat` accumulator, which retires the export re-emit hack
8. [ ] **Config parity Batch F — SDXL conditioning** — also what StoryFlow's `xlMagic` needs
9. [ ] **README polish** — screenshots, demo GIF
