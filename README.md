...
- **New Project…** creates the folder and seeds both files, so a project starts in the app rather than by copying someone else's folder and editing it. The eight fragments arrive with the leading and trailing spaces the format requires already correct — the one thing nobody should have to get right by typing — and the two Draw Things configs are assigned from your own saved `#config` variables, matched by name where the intent is unambiguous and left blank where it isn't
- Edits a project folder's `bible.json` and `configs.json`: a **cast table** (name, identity, wardrobe, slate, line, voice, pinned seed) and a **staging** panel (the eight shared prose fragments, the negative prompt, the two canvas sizes, the dialogue pacing)
