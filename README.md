...
1. [ ] **StoryFlow 260723 Phase 2 — author the new instructions** — the schema table and the generic step card that renders any entry from it have shipped, so `concat`, `wildcard`, `sweep`, `size`, `enhance`, `approve`, `framesDialog` and the long-passthrough backlog are authorable from the add-step menu. What is left is editor-asset parity (`poseJSON` shortcuts are preserved but not editable) and the real exit criterion: author a project in Tanque Studio, export it, and run it in Draw Things' own pipeline. `interrogate` is deliberately excluded and stays passthrough-only
2. [ ] **Video handling — the general pass** — the DT Project Browser now plays clips (see Completed). Still open: audio (Draw Things stores an audio tensor per clip that Tanque Studio ignores), and a wider look at how series are handled outside the browser
...
4. [ ] **Config parity Batch F — SDXL conditioning** — also what StoryFlow's `xlMagic` needs
