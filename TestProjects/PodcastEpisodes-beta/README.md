...
| **A folder** | `~Pictures/PodcastEpisodes-beta/anchors/` — **create it before the first run.** |
...
`~Pictures/PodcastEpisodes-beta/anchors/` does not exist, phase A renders six stills and saves none of
...
1. Create `~Pictures/PodcastEpisodes-beta/anchors/`.
...
5. Run the StoryFlow script in Draw Things and paste in **`PodcastEpisodes-beta.pipeline.json`**.
...
|---|---|---|
| `PodcastEpisodes-beta.json` | the **Editor project** — `{projectName, items, …}`, shortcuts unexpanded | the StoryFlow Editor, or Tanque Studio |
| `PodcastEpisodes-beta.pipeline.json` | the **instruction array** — a flat `[{key: value}, …]`, shortcuts resolved | paste into Draw Things' StoryFlow script |
...
| `build_project.py` | `bible.json` + `configs.json` → `PodcastEpisodes-beta.json`. Also writes a copy to `TanqueStudioTests/Fixtures/podcast-episodes-beta.json`, which is what the Swift round-trip tests check. |
| `verify_project.py` | Pre-flight. `--strict` also fails on remaining placeholders. |
| `PodcastEpisodes-beta.json` | **Generated.** The Editor project. For the Editor and Tanque Studio. Do not hand-edit. |
| `PodcastEpisodes-beta.pipeline.json` | **Generated.** The instruction array. This is the one Draw Things wants. |
