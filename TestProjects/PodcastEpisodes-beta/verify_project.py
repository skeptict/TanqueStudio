...
def default_project_path() -> Path:
    """The emitted project, named by configs.json's project block — same source build_project.py
    uses, so the two cannot disagree about which file they mean."""
    try:
        cfg = json.loads(DEFAULT_CONFIGS.read_text(encoding="utf-8"))
        return HERE / f"{cfg['project']['outputBasename']}.json"
    except (OSError, json.JSONDecodeError, KeyError, TypeError):
        return HERE / "project.json"

# NO FRAME CAP, in either engine (2026-08-11, Ned's call).
#
# Tanque Studio used to clamp framesDialog's spoken count at 257 while StoryflowPipeline.js
# clamped nothing, so one project rendered different lengths in the two engines past 27 spoken
# words. That clamp is gone. The real limit is what a given model at a given canvas size will
# render — for Draw Things+, what it renders without extra cost — which varies by both and cannot
# be anticipated by a constant here.
#
# So this file reports the frame count and says nothing about whether it is too large; the number
# and its duration are the useful output, and the judgement is the author's.
...
def check_seed_sweep(parsed: list[tuple[int, str, object]], r: Report) -> None:
    """Pinned seeds exist so a rejected anchor can be regenerated identically. Two characters
    sharing one defeats that for both — regenerate either and you get the other's roll — and it
    is the obvious slip when a character is added by copying the row above."""
    for i, t, obj in parsed:
        if t != "sweep" or obj.get("paramName") != "seed":
            continue
        cards = obj.get("cards") or []
        seen: dict[str, list[int]] = {}
        for pos, card in enumerate(cards):
            seen.setdefault(str(card), []).append(pos)
        dupes = {seed: at for seed, at in seen.items() if len(at) > 1}
        if dupes:
            detail = "; ".join(f"{seed} at characters {at}" for seed, at in sorted(dupes.items()))
            r.warn(f"item {i} (sweep): duplicate pinned seeds — {detail}. Each character wants "
                   f"its own, or regenerating one anchor reproduces the other's.")
...
def main() -> int:
    ...
    ap.add_argument("project", nargs="?", type=Path, default=default_project_path())
